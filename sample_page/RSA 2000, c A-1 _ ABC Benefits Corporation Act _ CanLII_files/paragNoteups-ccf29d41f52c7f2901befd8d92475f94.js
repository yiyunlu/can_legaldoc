function createParagTools(paragNoteupsUrl, concatId, origin, translationUrl, lang, citationText, tinyUrl, forLegislations) {
		...
		if (dropdownItem == null || dropdownItem.length == 0) {
			...
			$.get("/noteups/paragNoteups/menu?lang=" + lang + "&concatId=" + concatId + "&parag=" + paragNum + "&origin=" + origin + "&translationUrl=" + translationUrl + "&forLegislations=" + forLegislations, function(data) {
				...
				dropdownMenu.find("a.copy-text").on("click", function (e) {
					...
					Tipped.create($(this), "<div class='bootstrap'>" + getCopiedMsg(getLanguage()) + "</div>", { cache: false, showOn: false, afterHide : function(content, element) {
					...
					let text = Array.from(paragWrapper.children).filter(child => !child.classList.contains('unselectable')).reduce((prev, c) => prev += c.innerText, "");
					CanliiAnalytics.copyParagraphToolClicked(paragNumber, "text", text);
			    });
				...
				dropdownMenu.find("a.copy-source").on("click", function (e) {
					...
					let sourcePart = citationText + ", " + getAtMsg(getLanguage()) + " para " + paragNumber + ", <" + paragUrl + ">, " + getRetrievedOnMsg(getLanguage()) + getFormattedNowDate();
					...
					if (forLegislations != undefined && forLegislations == true) {
						...
						sourcePart = citationText + ", " + getSectionReferenceMsg(getLanguage()) + " " + section + (subsection == null ? "" : "(" + subsection + ")") + ", <" + paragUrl + ">, " + getRetrievedOnMsg(getLanguage()) + getFormattedNowDate();
					}
				    
				    copyStringToClipboard(sourcePart);
					Tipped.create($(this), "<div class='bootstrap'>" + getCopiedSourceMsg(getLanguage()) + "</div>", { cache: false, showOn: false, afterHide : function(content, element) {
						// remove the tipped after it's hidden, so the styles are reset if the user decides to switch theme
						Tipped.remove(element);
					} });
					Tipped.show(this);
					
					CanliiAnalytics.copyParagraphToolClicked(paragNumber, "citation", sourcePart);
				});
				...
				dropdownMenu.find("a.copy-link").on("click", function (e) {
					...
					Tipped.create($(this), "<div class='bootstrap'>" + getCopiedLinkMsg(getLanguage()) + "</div>", { cache: false, showOn: false, afterHide : function(content, element) {
						// remove the tipped after it's hidden, so the styles are reset if the user decides to switch theme
						Tipped.remove(element);
					} });
					Tipped.show(this);
					
					CanliiAnalytics.copyParagraphToolClicked(paragNumber, "link", paragUrl);
				});
			})
		    .fail(function(jqXHR) {
			    console.error(jqXHR);
    ...
    $.get(paragNoteupsUrl, function(data) {
		...
		if ((forLegislations && !isIe() && addHeatmap) || !forLegislations) {
			...
			let scrollMarker = document.querySelector(".heatmap");
...
function insertScrollMarkers(lang, forLegislations, scrollMarkersToAdd) {
	for (let i=0; i<scrollMarkersToAdd.length; i++) {
		...
		scrollMarkerChild.addEventListener("click", function(e) {
			...
			const desiredOffset = scrollMarkersToAdd[i]['spanTop'] - getHeaderOffset();
	    ...
	    if (!isTouchDevice()) {
			createScrollMarkerTip(getLanguage(), scrollMarkersToAdd[i]['start'], scrollMarkerChild, scrollMarkersToAdd[i]['end'], scrollMarkersToAdd[i]['citedCountDisplay'], forLegislations);
	    }
	}
}
...
function calculateScrollMarkers() {
	...
	let scrollMarkers = document.querySelectorAll('div.heatmap span');
...
function computeScrollMarkerHeightAndPos(spanTop, viibesMarkerChild) {
	let containerHeight = $("#wrap").outerHeight(true);
	let headerHeight = $("#canlii-header").outerHeight(true);
